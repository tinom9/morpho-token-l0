...
task('lz:set-rate-limits', 'Set specified rate limits for the contracts')
    .addParam('networks', 'Comma-separated list of networks to set the rate limits on')
    .setAction(async ({ networks }, hre) => {
        const networkNames = networks.split(',').map((networkName: string) => networkName.trim())
